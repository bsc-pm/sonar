...
AC_DEFUN([AC_CHECK_OVNI],
	[
		AC_ARG_WITH(
			[ovni],
			[AS_HELP_STRING([--with-ovni@<:@=DIR@:>@], [specify the ovni library installation prefix])],
			[ac_use_ovni_prefix="${withval}"],
			[ac_use_ovni_prefix="yes"]
		)
		...
		if test x"${ac_use_ovni}" != x"yes" ; then
			AC_MSG_ERROR([ovni is required but not found])
		fi
